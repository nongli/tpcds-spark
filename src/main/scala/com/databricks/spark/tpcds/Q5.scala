...
// Modifications: "datecol + 14 days" --> "date_add(datecol, 14)"
// Modifications: "stringcol || stringcol" --> "concat(stringcol, stringcol)"
// [UNSUPPORTED]: rollup
object Q5 {
  def query(tpcds: TpcdsBenchmark): TpcdsBenchmark.Query = {
    val year = tpcds.uniformRand(2000, 1998, 2002)
    val date = tpcds.uniformRand(23, 1, 30)
    val sales_date = s"$year-08-$date"
